...
func (repositorio Usuarios) Buscar(nomeOuNick string) ([]modelos.Usuario, error) {
    ...
    linhas, erro := repositorio.db.Query("select id, nome, nick, email, "+
        "criadoEm from usuarios where nome LIKE ? or nick LIKE ?", nomeOuNick, nomeOuNick)
    if erro != nil {
        return nil, erro
    }
    defer linhas.Close()
    ...
    for linhas.Next() {
        ...
        if erro = linhas.Scan(&usuario.ID, &usuario.Nome, &usuario.Nick, &usuario.Email,
...
func (repositorio Usuarios) Seguir(usuarioID, seguidorID uint64) error{
	statement, erro := repositorio.db.Prepare(
		"insert ignore into seguidores (usuarioID, seguidorID) values (?, ?)",
	)
	if erro != nil {
		return erro
	}
...
func (respositorio Usuarios) PararDeSeguir(usuarioID, seguidorID uint64) error{
...
// BuscarSeguindo traz todos os usuarios que um usuário está seguindo
...
// BuscarSenha traz a senha do usuário pelo id.
